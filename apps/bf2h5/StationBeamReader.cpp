...
StationBeamReader::StationBeamReader(BF2H5 *parent, bool socket_mode)
	: socklen(sizeof(incoming_addr)), rawfile(0), itsParent(parent), socketmode(socket_mode), 
		memAllocOK(true), blockHeaderSize(sizeof(BFRawFormat::BlockHeader))
{
	bigendian = BigEndian();
}
...
bool StationBeamReader::connectSocket(unsigned int port_number) {
	...
	{
#ifdef DEBUGGING_MESSAGES
		cerr << "StationBeamReader::connectSocket: socket creation failed" << endl;
#endif
		return false;
	}
...
bool StationBeamReader::readFirstDataBlock(BFRawFormat::BlockHeader &first_block_header, BFRawFormat::Sample *sample_data, size_t data_block_size) {
	dataBlockSize = data_block_size;
	int64_t read_bytes = receiveBytes(reinterpret_cast<char *>(&first_block_header), sizeof(BFRawFormat::BlockHeader));
	if (read_bytes > 0) {
...
int64_t StationBeamReader::receiveBytes(void *storage, int64_t nrOfBytesToRead) {
...
void StationBeamReader::printHeaderParameters(BFRawFormat::BFRaw_Header &header) {
	cout << "Magic number: " << header.magic << endl
			...
			<< "Polarizations : " << header.nrPolarizations << endl
			<< "Subbands: " << header.nrSubbands << endl
