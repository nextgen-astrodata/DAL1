...
void HDF5Writer::writeSubband (long int blockNr,
			       uint8_t subband,
...
*/
void HDF5Writer::writeData(void)
{
  while (!stopWriting) {
...
void HDF5Writer::openRawFile( const char* filename )
{
  ...
  /* See how many bytes there are in the file */
  file_byte_size = static_cast<unsigned long int>(rawfile->tellg())-2;
