/*-------------------------------------------------------------------------*
 | $Id:: TBBraw2h5.cc 2770 2009-07-02 17:52:38Z horneffer                  $ |
 *-------------------------------------------------------------------------*
 ***************************************************************************
 *   Copyright (C) 2009                                                    *
...
void socketReaderThread(int port, string ip, double startTimeout, 
  ...
  while (ImRunning){
	...
	boost::mutex::scoped_lock lock(writeMutex);
	if (numWaiting > maxWaitingFrames) { maxWaitingFrames=numWaiting; };
	numWaiting=0;
      };
    };
    FD_ZERO(&readSet);
...
bool readFromSocket(int port, string ip, float startTimeout, 
  ...
  while ((noRunning>0) || (inBufStorID != inBufProcessID) ){
    ...
    if (tmpint > maxCachedFrames) { maxCachedFrames = tmpint; };
    processingID = inBufProcessID+1;
    if (processingID >= INPUT_BUFFER_SIZE) { processingID -= INPUT_BUFFER_SIZE;};
...
bool readFromFile(string infile, bool verbose=false){
  ...
  if (verbose) { cout << "TBBraw2h5::readFromFile reading in " << numblocks << " blocks." << endl; };
  ...
  for (i=0; i<numblocks; i++){
    ...
    if ( (size != TBB_FRAME_SIZE )  ) {
      if (verbose) { cout << "TBBraw2h5::readFromFile Cannot read in block: " << i << endl; };
      fclose(fd);
      return false;
    };
