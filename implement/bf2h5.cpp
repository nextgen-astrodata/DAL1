...
BF2H5::~BF2H5()
{
//	cout << "destructor BF2H5 executing" << endl;
	delete itsReader;
	delete itsWriter;
...
void BF2H5::blockComplete(long int blockNr) {
	for (bufferTracker::iterator it = itsBufferTracker.begin(); it != itsBufferTracker.end(); ++it) {
		if (it->second == blockNr) {
			/*
#ifdef DEBUGGING_MESSAGES
		cout << "freeing sample buffer " << static_cast<int>(it->first) << endl;
#endif
			*/
//		memset(itsSampleBuffers[it->first], 0, oneBlockdataSize * sizeof(BFRawFormat::Sample));
		it->second = -1;
		return;
		}
	}
...
bool BF2H5::switchReadBuffer(long int block_nr) {
	for (bufferTracker::iterator it = itsBufferTracker.begin(); it != itsBufferTracker.end(); ++it) {
		if (it->second == -1) { // not in use
			/*
#ifdef DEBUGGING_MESSAGES
		cout << "switching to read buffer " << static_cast<int>(it->first) << endl;
#endif
			*/
			it->second = block_nr;
	...
	cout << "read buffer " << static_cast<int>(it->first) << " used by block " << it->second << endl;
		}
#endif
		*/
	}
...
void BF2H5::start(void) {
	...
	if (result)	{
     ...
     //         cout << "calculator mutex locked by " << itsCalculator->mutex_locked_by << endl;
						}
							// stop all threads
