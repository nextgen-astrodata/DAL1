/*-------------------------------------------------------------------------*
 | $Id:: TBBraw.cpp 2770 2009-07-01 12:36:06Z horneffer                  $ |
 *-------------------------------------------------------------------------*
 ***************************************************************************
 *   Copyright (C) 2009                                                    *
...
namespace DAL { // Namespace DAL -- begin
  ...
  void TBBraw::destroy() {
    ...
    if (dataset_p != NULL) { delete dataset_p; dataset_p=NULL; };
    delete [] stationBuf;
    delete [] dipoleBuf;
  }
  ...
  int TBBraw::getDipoleIndex(TBB_Header *headerp){
	...
	break;
      ...
      if (dipoleBuf[i].array == NULL) { break; };
    }
    if (dipoleIndex != -1) {
      return dipoleIndex;
    } else {
  ...
  int TBBraw::createNewDipole(TBB_Header *headerp){
	...
	break;
      ...
      if (stationBuf[i].group == NULL) { break; };
    }
    if (stationIndex == -1) {
    ...
    for (dipoleIndex=0; dipoleIndex<MAX_NO_DIPOLES; dipoleIndex++) {
      if (dipoleBuf[dipoleIndex].array == NULL) { break; };
    };
  ...
  int TBBraw::createNewStation(TBB_Header *headerp){
    ...
    for (stationIndex=0; stationIndex<MAX_NO_STATIONS; stationIndex++) {
      if (stationBuf[stationIndex].group == NULL) { break; };
    };
