...
int get_attributes (hid_t const &file_id)
{
  ...
  if (dataset_id > 0) {
    ...
    try {
      status = DAL::h5get_attribute (sample_frequency,"SAMPLE_FREQ",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (time,"TIME",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (sample_number,"SAMPLE_NR",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (samples_per_frame,"SAMPLES_PER_FRAME",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (data_length,"DATA_LENGTH",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (nyquist_zone,"NYQUIST_ZONE",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (antenna_position,"ANT_POSITION",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
    ...
    try {
      status = DAL::h5get_attribute (antenna_orientation,"ANT_ORIENTATION",dataset_id);
    } catch (std::string message) {
      cerr << message << endl;
      nofFailedTests++;
    }
